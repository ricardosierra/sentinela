...
- [x] **As três supressões de detekt saíram, sem afrouxar o `detekt.yml`** — os três arquivos carregavam `@file:Suppress("TooManyFunctions")` e a tela Proteção ainda tinha `@Suppress("LongMethod")` sobre uma composta de 114 linhas. As supressões existiam porque a tela inteira morava num arquivo só; com as seções separadas, elas deixaram de ter motivo e foram removidas em vez de renegociadas no `detekt.yml` compartilhado
- [x] **Import morto não sobrevive à refatoração** — extrair uma seção tira o código do arquivo e deixa o `import` dele para trás: sobraram 91 linhas apontando para símbolos que os três arquivos não usam mais — a animação infinita inteira em `RoleStepScreen.kt`, `ModalBottomSheet` e seis ícones em `HomeScreen.kt`. O `detekt` não pega isso, porque `UnusedImports` vem desligado por padrão no 1.23 (exige resolução de tipo), então a pista falsa duraria até alguém abrir o arquivo procurando um componente que não está mais lá. `getValue` e `setValue` ficaram onde a delegação `by` os usa sem citar o nome
- [x] **A precedência dos avisos da home saiu do Compose** — `avisosDaHome` era uma composta de 56 linhas, e só era composta porque resolvia texto de recurso ali mesmo; a regra em si — papel de triagem ausente, agenda negada, histórico desligado, falha de leitura, papel de discador perdido — nunca dependeu de composição. Virou função pura com 12 casos de JVM em `HomeWarningsTest`, sem Robolectric e sem árvore de semântica. Antes, afirmar a ORDEM dos cinco motivos exigia montar a tela e ler texto, e o teto de dois avisos escondia do teste tudo a partir do terceiro
- [x] **Nenhuma regra de bloqueio de chamada mora na interface** — auditoria dos três arquivos e do resto de `ui/`: toda decisão de "essa chamada toca ou não" continua em `domain/CallDecisionEngine`, e o que a interface faz com `OriginPolicy`, `BlockMode` e `FallbackPolicy` é escolher qual opção aparece marcada e traduzir motivo em rótulo. Nada foi movido porque nada estava fora do lugar
- [x] **A pré-visualização da home mentia sobre a barra inferior** — o dublê de barra usado pela pré-visualização marcava Lista pessoal e Histórico como desabilitados com o aviso `nav_unavailable`, congelando um estado de fase anterior; a barra real, montada em `SentinelaNavHost.kt`, navega para os quatro destinos há tempo. Como a barra de verdade nunca passou por aquele código, a divergência durou — e a única imagem da home que alguém olha antes de mexer nela era a errada. Corrigido; nada de produção muda
- [x] **`PASSO_DO_PAPEL` valia 1 num arquivo e 0 em outro** — em `RoleStepScreen.kt` era o número de exibição (o "1" de "passo 1 de 6") e em `OnboardingRoute.kt` é o índice do passo no fluxo. Dois nomes iguais com valores diferentes no mesmo pacote; o primeiro virou `NUMERO_DE_EXIBICAO_DO_PASSO`
- [x] **Medidas que eram empréstimo viraram medidas próprias** — o convite de avaliação espaçava título, corpo e botões com as constantes dos BLOCOS da home, e o cartão vazio usava o vão ENTRE os dois cartões de estatística. Coincidência de valor, não de motivo: mexer no espaçamento dos blocos mudaria o interior de uma folha modal que não tem bloco nenhum. Mesmos valores, nomes que dizem o que separam
...
- [x] **Cobertura atual medida, para a decisão de mínimo** — execução limpa de 2026-08-23 sobre os 895 testes unitários (78 classes, zero falhas, zero pulados): **linha 96,31%**, instrução 94,21%, método 93,04%, classe 91,04% e **ramo 83,80%** — ramo é a métrica mais apertada. O denominador é o bloco `kover` de `app/build.gradle.kts`. Fica registrado como número, não como limiar: nenhum gate novo foi criado e o `minBound(80)` existente não foi mexido
