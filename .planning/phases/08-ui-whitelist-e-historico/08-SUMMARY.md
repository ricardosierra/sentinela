...
Abas de Home, Whitelist, Histórico e Proteção com bottom bar (`99c815e`).
...
1. **Backup vazio** — a rota gravava a constante `{"whitelist":[]}` no arquivo escolhido pelo
   ...
   nele. Corrigido em `893fb63`.
2. **Importação inerte** — a rota lia o arquivo e descartava o conteúdo (`skip for
   simplicity`). O botão Importar não importava nada. Corrigido em `893fb63`, com a
...
3. **Duplicado e inválido em silêncio** — `addOrEdit` fazia `return@launch` nos dois casos; o
   diálogo fechava como se tivesse gravado. Corrigido em `893fb63` com canal de aviso.
4. **Filtro por decisão inexistente** — só havia período. Corrigido em `851f3c2`.
5. **Tempo falso** — toda linha do histórico exibia a constante `"Agora"`, com o comentário
   ...
   ter acontecido inteiro no último minuto. Corrigido em `851f3c2`.
6. **Reason code cru na tela** — a legenda imprimia `entry.reason.name`, ou seja
   `UNKNOWN_NUMBER`, em inglês e fora de `strings.xml`. Corrigido em `851f3c2`.

Além disso, esta fase deixou dois rastros fora do escopo dela, também corrigidos:

- **Regressão do invariante central de telecom** — o commit `901d9d3` ("Wave 08-03 Tela
  Whitelist") reescreveu `UnknownCallScreeningService`, criando um segundo ponto de resposta
  ao sistema. Corrigido em `cabfbd1`, com regressão coberta por teste. Detalhes em
  `09-VERIFICATION.md`.
- **557 arquivos de `.venv/` versionados** no mesmo commit. Removidos em `5a92e9f`.
