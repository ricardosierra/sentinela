...
## Os 3 fatos que sustentam o produto

1. **Sem ser discador padrão, `onScreenCall()` só recebe chamadas de números fora da agenda.**
   Contatos tocam normalmente sem o app pedir `READ_CONTACTS`. É o que torna o MVP viável com
   permissão mínima. (developer.android.com/develop/connectivity/telecom/dialer-app/screen-calls)

2. **`respondToCall` tem janela de ~5 s** — depois disso a chamada segue como permitida.
   Por isso o orçamento interno de p95 < 200 ms e a política de fallback explícita.

3. **`CallResponse` oferece exatamente os controles que o produto precisa:**
   ...
   obrigatória na Phase 4 + validação física na Phase 7.
...
| detekt 1.23.8 pré-data o Kotlin 2.4 | Validar na Phase 1; fallback: rodar detekt via CLI fora do Gradle |
